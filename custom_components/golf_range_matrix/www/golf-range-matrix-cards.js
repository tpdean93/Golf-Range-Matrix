...
class NovaShotTracerCard extends HTMLElement {
  ...
  render() {
    ...
    const impactGlowId = `impact-glow-${this._renderId}`;
    ...
    this.innerHTML = `<ha-card><div class="shell">
      ...
      <div class="stage"><svg viewBox="0 0 390 252" preserveAspectRatio="none" aria-label="Animated shot tracer driving range grid"><defs><linearGradient id="${tracerId}" x1="0" x2="1" y1="1" y2="0"><stop offset="0" stop-color="#f7ff5c"/><stop offset="0.44" stop-color="#38f8ff"/><stop offset="1" stop-color="#b36bff"/></linearGradient><radialGradient id="${impactGlowId}"><stop offset="0" stop-color="#fff"/><stop offset="1" stop-color="#38f8ff" stop-opacity="0"/></radialGradient><filter id="${glowId}"><feGaussianBlur stdDeviation="4" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs>
        ...
        <path class="ghost" d="${shadow}"/><path id="${pathId}" class="pathTemplate" d="${curve}"/><path class="curve halo" d="${curve}"/><path class="curve animated" d="${curve}"/><circle class="impactGlow" cx="${startX}" cy="${startY}" r="30"/><circle class="tee" cx="${startX}" cy="${startY}" r="5"/><circle class="movingBall" r="5"><animateMotion dur="3s" repeatCount="indefinite" rotate="auto" calcMode="spline" keyTimes="0;1" keyPoints="0;1" keySplines=".2 0 .2 1"><mpath href="#${pathId}"/></animateMotion><animate attributeName="opacity" values="0;1;1;0" keyTimes="0;0.08;0.86;1" dur="3s" repeatCount="indefinite"/></circle><circle class="landPulse" cx="${endX}" cy="${landY}" r="5"/><text class="impactLabel" x="151" y="248">IMPACT</text><text class="flightLabel" x="${this.clamp(endX - 54, 18, 292)}" y="${this.clamp(landY - 12, 24, 168)}">${novaEsc(offlineLabel)}</text>
      </svg></div>
